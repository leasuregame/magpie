...
void CCEGLViewProtocol::setDesignResolutionSize(float offsetX, float offsetY, float width, float height, ResolutionPolicy resolutionPolicy)
{
    ...
    m_obDesignResolutionSize.width += offsetX;
    m_obDesignResolutionSize.height += offsetY;

    // calculate the rect of viewport    
    float viewPortW = m_obDesignResolutionSize.width * m_fScaleX;
    float viewPortH = m_obDesignResolutionSize.height * m_fScaleY;
    float viewPortX = (m_obScreenSize.width - viewPortW) / 2 - offsetX;
    float viewPortY = (m_obScreenSize.height - viewPortH) / 2 - offsetY;
    
    m_obViewPortRect.setRect(viewPortX, viewPortY, viewPortW, viewPortH);

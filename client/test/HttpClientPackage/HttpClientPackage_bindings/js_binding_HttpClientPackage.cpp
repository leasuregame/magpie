...
JSBool js_js_binding_HttpClientPackage_HttpClientPackage_callback(JSContext *cx, uint32_t argc, jsval *vp)
{
	jsval *argv = JS_ARGV(cx, vp);
	JSBool ok = JS_TRUE;
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
...
JSBool js_js_binding_HttpClientPackage_HttpClientPackage_GetResponseCode(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy; JS_GET_NATIVE_PROXY(proxy, obj);
	HttpClientPackage* cobj = (HttpClientPackage *)(proxy ? proxy->ptr : NULL);
	JSB_PRECONDITION2( cobj, cx, JS_FALSE, "Invalid Native Object");
	if (argc == 0) {
		std::string ret = cobj->GetResponseCode();
		jsval jsret;
...
void js_register_js_binding_HttpClientPackage_HttpClientPackage(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
		...
		JS_FN("HttpPostRequest", js_js_binding_HttpClientPackage_HttpClientPackage_HttpPostRequest, 2, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FN("callback", js_js_binding_HttpClientPackage_HttpClientPackage_callback, 2, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FN("GetResponseCode", js_js_binding_HttpClientPackage_HttpClientPackage_GetResponseCode, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),

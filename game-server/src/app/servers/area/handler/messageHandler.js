...
var ADD_FRIEND_MESSAGE, DELETE_FRIEND_MESSAGE, Handler, MAX_POWER_VALUE, SYSTEM, achieve, async, changeGroupNameAndSort, checkSystemOptions, configData, dao, eUtil, isFinalStatus, logger, mergeMessages, playerManager, resData, sendMessage, table, updateGiveCount, updateReceiveCount, utility, _,
...
Handler.prototype.messageList = function(msg, session, next) {
  ...
  return async.parallel([
    function(cb) {
      ...
      return dao.message.fetchMany({
        where: " receiver = " + playerId + " and (          (type = " + configData.message.MESSAGETYPE.SYSTEM + " and DATE(validDate) >= '" + today + "') or           (type = " + configData.message.MESSAGETYPE.MESSAGE + ")        )",
        orderby: ' createTime DESC '
      }, cb);
    }, function(cb) {
      return dao.message.fetchMany({
...
Handler.prototype.sysMsg = function(msg, session, next) {
  var content, options, receiver, validDate,
    _this = this;

  content = msg.content;
  options = msg.options;
  validDate = msg.validDate;
  if (msg.playerId && _.isNumber(msg.playerId) && msg.playerId > 0) {
  ...
  return async.waterfall([
    function(cb) {
      return checkSystemOptions(options, cb);
    }, function(cb) {
      if (receiver !== SYSTEM) {
    ...
    return sendMessage(_this.app, (receiver === -1 ? null : receiver), {
      route: 'onMessage',
      msg: res.toJson()
    }, '邮件发送成功', function(err, data) {
      if (!err) {
        data.msg = {
          msgId: res.id,
          tip: data.msg
...
checkSystemOptions = function(options, cb) {
  var hasRightProperties, hasRightRewards, isAcceptLength, isObject, rewardTypes, wrongKeys;

  isObject = _.isObject(options);
  hasRightProperties = _.has(options, 'title') && _.has(options, 'sender') && _.has(options, 'rewards');
  if (isObject) {
    isAcceptLength = JSON.stringify(options).length <= 1024;
  }
  rewardTypes = ['gold', 'money', 'spirit', 'skillPoint', 'energy', 'fragments', 'elixir', 'superHonor', 'powerValue', 'cardArray', 'speaker', 'honor'];
  wrongKeys = _.keys(options.rewards).filter(function(k) {
    return __indexOf.call(rewardTypes, k) < 0;
  });
  if (isObject && hasRightProperties) {
    hasRightRewards = wrongKeys.length === 0;
  }
  if (isObject && hasRightProperties && hasRightRewards && isAcceptLength) {
    return cb();
  } else {
    return cb({
      code: 501,
...
Handler.prototype.handleSysMsg = function(msg, session, next) {
  ...
  playerId = session.get('playerId');
  msgId = msg.msgId;
  incValues = function(obj, options, done) {
    var data, k, _i, _len, _ref;

    data = options.rewards || options;
    _ref = _.keys(data);
    ...
    if (_.has(data, 'spirit')) {
      obj.incSpirit(data.spirit);
    }
    if (_.has(data, 'cardArray') && data.cardArray.length > 0) {
      data.cardArray.forEach(function(c) {
        return c.playerId = obj.id;
      });
      return eUtil.createCards(data.cardArray, function(err, cards) {
        if (err) {
          return done(err);
        } else {
          obj.addCards(cards);
          data.cardArray = cards.map(function(c) {
            return c.toJson();
          });
  ...
  return async.waterfall([
    function(cb) {
      ...
      return incValues(player, options, cb);
    }, function(data, cb) {
      if (_.isArray(data.cardArray) && data.cardArray.length > 0) {
        data.cardArray.forEach(function(c) {
          if (c.star === 5) {
            achieve.star5card(player);
          }
          if (c.star === 6) {
            achieve.star6card(player);
          }
          if (c.star === 7) {
            return achieve.star7card(player);
          }
        });
      }
      return cb(null, data);
